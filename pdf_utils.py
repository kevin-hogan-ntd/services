...
def load_pronunciations(pron_path: str = DEFAULT_PRONUNCIATION_PATH):
    """
    Returns {word: "PHONEME  |  ALT-PHONEME"}  (lower-cased keys)
    """
...
def deduplicate_text(text):
    """Remove artificial doubled letters that come from stylized PDFs."""
    def fix_word(word):
...
def load_dictionary(dictionary_path):
    """Load newline-separated dictionary words → set()"""
...
def is_all_doubled(word):
    """True if every character appears twice in succession: AAbbcc …"""
...
def extract_non_dictionary_words(
    ...
    extra_pronunciations=None,         # ← Sheet pronunciations
):
    # -------------------------------------------------------------
    # 2-A  Load dictionaries and pronunciation maps
    # -------------------------------------------------------------
    dictionary     = load_dictionary(dictionary_path)
    dict_pron      = load_pronunciations(pronunciation_path)        # CMU dict
    sheet_pron     = {k.lower(): v for k, v in (extra_pronunciations or {}).items()}

    def annotate(word: str) -> str:
        """
        Return 'word  —  sheetPron / dictPron' (whichever exist).
        Sheet pronunciation is shown first.
        """
        parts = []
        if word in sheet_pron:
            parts.append(sheet_pron[word])
        if word in dict_pron:
            parts.append(dict_pron[word])
        return f"{word}  —  {' / '.join(parts)}" if parts else word
    ...
    # -------------------------------------------------------------
    # 2-B  Walk through the text and collect stats
    # -------------------------------------------------------------
    paragraphs        = text.split("\n\n")
    word_origins      = {}           # word → section label
    seen_words        = set()
    ...
    for para in paragraphs:
        ...
        if hdr:
            lbl = hdr.group(1)
            current_section = (
                "pkg"    if "pkg"    in lbl else
                "anchor" if "anchor" in lbl else
                None
            )
        ...
        for word in re.findall(r"\b[a-zA-Z]+\b", para):
            ...
            if len(w) >= 12:
                long_words.add(w)
            if (
                w in dictionary                           # known word
                or w in seen_words                        # processed already
                or is_all_doubled(w)                      # stylized duplication
                or duplicate_pattern.fullmatch(w)         # aaaAAA pattern
            ):
                continue
            seen_words.add(w)
    ...
    with open(output_txt_path, "w", encoding="utf-8") as f:
        ...
        # infrequent dictionary words (<4 occurrences)
               # infrequent dictionary words (<4 occurrences)
        ...
        # least-frequent 150 known words with rank data
        f.write("\n\nleast frequent 150 known words (dictionary ∩ en_full.txt)\n")
        ranked = [
            (freq_map[w], w) for w in word_counter
